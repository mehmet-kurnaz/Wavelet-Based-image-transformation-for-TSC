...
df_metadata = pd.read_excel('dsets_ucr_ts_uv/ucr_dsets_all_metadata.xlsx')
df_ref_full_path = 'dsets_ucr_ts_uv/_benchmark_results/FreshPRINCE_accuracy.csv'
df_ref = pd.read_csv(df_ref_full_path)

ind_ref_path = 'dsets_ucr_ts_uv/PythonResampleIndices'
...
for clf_name in clfnames:
    ...
    for dset_name in dataset_names:
        ...
        for fold_ in range(n_re):
            ...
            # Eğitim seti sinyallerini normalize edip resize et
            X_train_resized = [np.interp(np.linspace(0, len(sig) - 1, tmstep), np.arange(len(sig)), 
                                (sig - np.min(sig)) / (np.max(sig) - np.min(sig) + 1e-6)) for sig in x_tr]
            X_train_resized = np.array(X_train_resized)
        
            # Fit sadece eğitim verisine
            gasf.fit(X_train_resized)
            mtf.fit(X_train_resized)

            print(fold_)

            def compute_mtf_signals(signals, tmstep):
                mtf = MarkovTransitionField(image_size=tmstep, n_bins=8)
                mtf_imgs = []
                for sig in signals:
                    sig_norm = (sig - np.min(sig)) / (np.max(sig) - np.min(sig) + 1e-6)
                    sig_resized = np.interp(np.linspace(0, len(sig_norm) - 1, tmstep), np.arange(len(sig_norm)), sig_norm)
                    mtf_img = mtf.fit_transform(sig_resized.reshape(1, -1))[0]
                    mtf_imgs.append(mtf_img)
                return np.array(mtf_imgs, dtype=np.float32)
            
            X_train_mtf_orig = compute_mtf_signals(x_tr, tmstep)
            X_test_mtf_orig = compute_mtf_signals(x_te, tmstep)
            
            
            def process_single_signal(signal, mtf_orig_img):
                signal_norm = (signal - np.min(signal)) / (np.max(signal) - np.min(signal) + 1e-6)
                sig_resized = np.interp(np.linspace(0, len(signal_norm) - 1, tmstep), np.arange(len(signal_norm)), signal_norm)
            
                gasf = GramianAngularField(image_size=tmstep, method='summation')
                rp = RecurrencePlot(threshold='point', percentage=20)
            
                gaf_orig = gasf.fit_transform(sig_resized.reshape(1, -1))[0]
                rp_orig = rp.fit_transform(sig_resized.reshape(1, -1))[0]
                rp_orig = resize(rp_orig, (tmstep, tmstep), mode='reflect', anti_aliasing=True)
            
                # DWT işlemleri
                coeffs = pywt.wavedec(signal_norm, 'db4', level=2)
                n_timestamps = tmstep
                ...
                gaf_dwt = gasf.fit_transform(c_resized.reshape(1, -1))[0]
                ...
                c = (c - np.min(c)) / (np.max(c) - np.min(c) + 1e-6)
                c_resized = np.interp(np.linspace(0, len(c) - 1, n_timestamps), np.arange(len(c)), c)
                mtf_dwt = mtf.fit_transform(c_resized.reshape(1, -1))[0]
                ...
                rp_dwt = resize(rp_img, (tmstep, tmstep), mode='reflect', anti_aliasing=True)
            
                # mtf_orig_img parametresi dışardan geliyor
                return np.stack([gaf_orig, rp_orig, mtf_orig_img, gaf_dwt, rp_dwt, mtf_dwt], axis=-1)
            
            # Paralel çalıştırırken mtf orig görüntüsünü de ver:
            X_train_img = np.array(Parallel(n_jobs=-1)(delayed(process_single_signal)(sig, mtf_orig) for sig, mtf_orig in zip(x_tr, X_train_mtf_orig)))
            X_test_img = np.array(Parallel(n_jobs=-1)(delayed(process_single_signal)(sig, mtf_orig) for sig, mtf_orig in zip(x_te, X_test_mtf_orig)))
            ...
            def split_into_raw_and_dwt(X):
                X_raw = X[..., :3]  # GAF, RP, MTF
                X_dwt = X[..., 3:]  # GAF-DWT, RP-DWT, MTF-DWT
                return X_raw, X_dwt
            X_train_raw, X_train_dwt = split_into_raw_and_dwt(X_train_img)
            X_test_raw, X_test_dwt = split_into_raw_and_dwt(X_test_img)

            input_shape = (tmstep, tmstep, 3)
            ...
            # Modeli birleştir
            input_raw = tf.keras.Input(shape=input_shape)
            input_dwt = tf.keras.Input(shape=input_shape)
            
            feat_raw = raw_model(input_raw)
            feat_dwt = dwt_model(input_dwt)
            
            # Özellikleri birleştir
            merged = layers.Concatenate()([feat_raw, feat_dwt])
            ...
            # Final model
            model = tf.keras.Model(inputs=[input_raw, input_dwt], outputs=output)
            ...
            history = model.fit(
                [X_train_raw, X_train_dwt], 
                y_tr_oh,
                validation_data=([X_test_raw, X_test_dwt], y_te_oh),
                epochs=10,
                verbose=0,
                batch_size=16,
            ...
            # Tahmin
            t0 = time()
            y_pred_proba = model.predict([X_test_raw, X_test_dwt])
            dur_pred = time() - t0
